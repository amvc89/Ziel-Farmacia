		...
		$('.cboProductos').selectize({
		    ...
		    render: {
		        item: function(item, escape) {
		            ...
		            $('#txtVenta').val( item.venta );
		            $('#lblVenta').html('Utilidad <span class="text-success">'+item.utilidad+'%</span>');
		            return '<div>' +
		                (item.nombre ? '<span class="ItemNomProd">' + escape(item.nombre) + '</span> ' : ' ') +
		                (item.lab ? '<ul class=" list-inline" ><li class="text-success" >Laboratorio: <b>' + escape(item.lab) + '</b></li><li class="text-success" >Precio Compra: <b>' + escape(item.compra) + '</b></li><li class="text-success" >Precio venta: <b>' + escape(item.venta) + '</b></li><li class="text-success" >Utilidad: <b>' + escape(item.utilidad) + '%</b></li></ul>' : ' ') +
		            '</div>';
		        ...
		        option: function(item, escape) {
		            ...
		            return '<div>' +
		                '<span class="cboNomProd">' + escape(item.nombre) + ' <small class="text-muted">Tipo: '+item.tipo+'</small></span>' +
		                (caption ? '<ul class=" list-inline" ><li class="text-success" >Laboratorio: <b>' + escape(item.lab) + '</b></li><li class="text-success" >Precio Compra: <b>' + escape(item.compra) + '</b></li><li class="text-success" >Precio venta: <b>' + escape(item.venta) + '</b></li><li class="text-success" >Utilidad: <b>' + escape(item.utilidad) + '%</b></li></ul>' : '') +
		            '</div>';
		        }
		    },
		});
...
function LoadItem( json ){
	if( json.data != undefined || json.data != null ){
		...
		for (var i = 0; i < json.data.length; i++) {
			...
			$('#lblVenta').html('Utilidad <span class="text-success">'+_item.flt_Utilidad+'%</span>');
...
function calcularUT( _tag , _elpc , _elpv ){
	if( !isNaN( _elpv ) && !isNaN( _elpc ) ){
		...
		$('#lblVenta').html('Utilidad <span class="text-success">'+_lauti+'%</span>');
	}
}
/* ---------------------------------------------------------- */
