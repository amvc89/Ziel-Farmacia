...
class Usuario extends MySQL{
    ...
    public function get_productos_lista( $filtro = '', $limit = '' ){
        ...
        $p          = array();
        #
        $sql = " SELECT *, g.var_Nombre as 'generico', p.var_Nombre as 'producto', p.tipo as 'tipo', 
    ...
    /* -------------------------------------------------------------------- */
    public function get_all_productos_simlex(){
        #traera todos los productos
        $sql = '';
        $data = array();
        $response = array();
        #
        #$sql = " SELECT p.*, l.int_Stock as 'stock' FROM `utb_productos` p INNER JOIN `utb_producto_lote` l ON l.int_IdProducto = p.int_IdProducto ";
        $sql = " SELECT * FROM `utb_productos` p INNER JOIN `utb_precio_producto` ps ON p.int_IdProducto = ps.int_IdProducto ";
        #
        $data = $this->QueryArray( $sql , MYSQL_ASSOC );
        if( is_array($data) ){
            $response['n'] = $this->RowCount();
    ...
    /* -------------------------------------------------------------------- */
    public function get_all_productos_simlex_ventas(){
        #traera todos los productos
        $sql = '';
        $data = array();
        $response = array();
        #
        #$sql = " SELECT p.*, l.int_Stock as 'stock' FROM `utb_productos` p INNER JOIN `utb_producto_lote` l ON l.int_IdProducto = p.int_IdProducto ";
        $sql = " SELECT *, pl.`var_Lote` as 'lote', pl.`int_Stock` 'stock',DATE_FORMAT(pl.dt_Vencimiento,'%d/%m/%Y') as 'vencimiento' FROM `utb_productos` p 
        INNER JOIN `utb_precio_producto` ps ON p.int_IdProducto = ps.int_IdProducto INNER JOIN `utb_producto_lote` pl on pl.`int_IdProducto` = p.int_IdProducto ";
        #
        $data = $this->QueryArray( $sql , MYSQL_ASSOC );
        if( is_array($data) ){
            $response['n'] = $this->RowCount();
    ...
    /* -------------------------------------------------------------------- */
    public function get_all_productos_filtro( $q ){
        #traera todos los productos
        $sql = '';
        $data = array();
        $response = array();
        #
        #$sql = " SELECT p.*, l.int_Stock as 'stock' FROM `utb_productos` p INNER JOIN `utb_producto_lote` l ON l.int_IdProducto = P.int_IdProducto and var_Nombre like '%".$q."%' ";
        $sql = " SELECT * FROM `utb_productos` p INNER JOIN `utb_precio_producto` ps ON p.int_IdProducto = ps.int_IdProducto AND p.var_Nombre like '%".$q."%' ";
        #
        $data = $this->QueryArray( $sql , MYSQL_ASSOC );
        if( is_array($data) ){
            $response['n'] = $this->RowCount();
